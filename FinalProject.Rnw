...
@
\linenumbers
\paragraph{}This data set is called the abalone data set. Abalone's are a form of sea snails whose age can be determined by boring into the shell and, through a "boring and time consuming" process, by counting the rings much like a tree. In order to save our conchologist friends time, we will attempt to predict the number of rings in the shell by measuring:
\ben
  \item Sex
  \item Length
  \item Diameter
...
Preprocessing:
<<>>=
data=read.csv("C:/Users/Rowe/Documents/My Repo/Final Project DM/abalone.data.txt", header = FALSE)
